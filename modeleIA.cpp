...
int score(Plateau plateau){
    ...
    for (int i = 0; i < 4; i++){
        for (int j = 0; j < 4; j++){
            s+=scoreNb(plateau[i][j]);
        }
    }
    return s;
}
/** Fonction scoreNb
 * Permet de calculer le score d'un nombre donné
 * @param nombre le nombre dont on veut calculer le score
 * @return  le score obtenu pour e nombre donné
 **/
...
int scoreNb(int nombre){
    ...
    return nombre+2*scoreNb(nombre/2);
}
...
/** Fonction ia
 * Permet de choisir le mouvement le plus judicieux
 * @param plateau la grille de jeu
 * @param rec le niveau maximum de réccurence 
 * @return Deplacement choix, une structure contenant un score et un plateau de jeu 
 **/
